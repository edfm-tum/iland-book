...
-   the "base" [management](https://iland-model.org/wiki/management.html) is a rather simple, but still versatile approach which allows a wide range of altering the forest in the model (extracting of trees, planting of trees, ...).

-   The [Agent Based management Engine (ABE)](https://iland-model.org/wiki/abe.html) is a newer variant. ABE is a powerful module that allows to simulate multiple management agents on a landscape. Agents can even react to changes in the environment by changing management plans and can apply a variety of different management activities in a dynamically scheduled manner. The setup and application of ABE, however, is somewhat complex.
...
iLand provides a simple way to assign every stand a STP at the start of the simulation: the ABE data file (`agentDataFile`). It reads data from the columns, `id` and `stp`, see <https://iland-model.org/wiki/abe-spatial-setup.html> and assigns the STPs given in column stp to the stand given in the id column.
...
Activities are the main building blocks of forest management in ABE. A detailed description is on the wiki: <https://iland-model.org/wiki/abe-activities.html>
...
action: function() {
  ...
  // see also: https://iland-model.org/apidoc/classes/abe/stand.html
  if (stand.volume > 300 ) {
...
Some statistics on the current state of the stand are available via the `stand` object, for example `stand.volume` is the standing volume (m3/ha) (see <https://iland-model.org/apidoc/classes/abe/stand.html> for more variables). More interesting is the `stand.flag()` in the example. Flags are stored as key-value pairs per stand; different stands can have different keys (or also the same keys) and can hold any (JavaScript) value, including JavaScript options. In the following, we discuss some example use of flags.
...
See the section on [Patches](https://iland-model.org/apidoc/classes/abe/patches.html) in the iLand documentation and the examples below for details.
