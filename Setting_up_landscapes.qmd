...
This chapter explains how to set up a "real" landscape. Information on other types of simulation set-ups (e.g., for growth evaluation tests) can be found [here](https://iland-model.org/wiki/simulation-extent.html#general-types-of-simulation-setups "iLand wiki: types of simulation set-ups").
...
-   Pixel values of *-2* = "forested outside pixel": continuous "shading" from the assumed forest outside of the project area is applied (LIF calculation), can act as source of external seeds, included in fetch calculations of the [wind module](https://iland-model.org/wiki/wind-module.html "iLand wiki: wind module")
...
More information on the description of the C model can be found [here](https://iland-model.org/wiki/soil-c-and-n-cycling.html "description carbon model") and on the parametrization process [here](https://iland-model.org/wiki/simplified-soil-parameterization.html "carbon model parametrization").
...
Within a stand the positions of the individual trees are derived by applying an pseudo-random approach (more information [here](https://iland-model.org/wiki/initialize-trees.html#tree-locations "iLand wiki: tree locations")) but can also be set using the single tree initialization approach (more information [here](https://iland-model.org/wiki/initialize-trees.html#single-tree-input-files "iLand wiki: single tree initialization")).

The same approach can be used to initialize saplings (i.e. trees \< 4m, see [here](https://iland-model.org/wiki/initialize-trees.html#initialize-saplings "iLand wiki: initialize saplings")).
...
To load a snapshot, one can use the mode "snapshot" in the project-file for the tree initialization (nodes *world.initialization.mode* and *world.initialization.file* in the project file). More information on snapshots can be found [here](https://iland-model.org/wiki/initialize-trees.html#snapshots "iLand wiki: snapshots").
...
The next sections give an overview of the two approaches, more detail on their implementation can be found [here](https://iland-model.org/wiki/external-seeds.html "iLand wiki: external seeds").
...
The wind module in iLand (see [here](https://iland-model.org/wiki/wind-module.html "iLand wiki: wind module") for more details) requires two types of landscape-specific inputs: a time series of wind events (with accompanying properties like wind direction and storm duration, see [here](https://iland-model.org/wiki/time-events.html "iLand wiki: time event scripts")) and a raster file at a resolution of 100x100 m which scales the global wind pattern to a local value ("topoModifier"-grid).
