...
For productivity experiments, tweak [species parameters](https://iland-model.org/species+parameter) to better match comparison data. Note that it is not expected to be perfect, because the data used in setting up these runs is also not perfect. Review [Seidl et al. 2012a Supplementary material](https://ars.els-cdn.com/content/image/1-s2.0-S0304380012000919-mmc1.pdf) for ideas of parameters to which productivity is most sensitive, and keep in mind which parameters might be most uncertain versus ones that are fairly well defined. Here are some examples:
...
-   **Are saplings surviving?**
If not, consider adjusting survival-related parameters (`sapMaxStress`,`SapStressThreshold`).
...
-   **Are forested conditions between maintained over time and generally covering the range of expected densities and structures?**
...
-   **Are saplings growing fast enough?** 
If not, consider adjusting `sapHeightGrowthPotential`. Usually, you will have parameterized this in the first place with empirical data, though, so it generally should align with expectations. Comparison data on sapling age versus height can help facilitate the calibration of this parameter.
...
**Evaluation strategy**\
Stand-scale regeneration simulations can be used to evaluate single-species behavior if comparison data is available, such as sapling growth over time or seedling establishment relative to distance from seed source (e.g., as in [Hansen et al. 2018](https://doi.org/10.1002/ecy.2181)).
