...
![The iLand user interface after startup. The next step is to select a landscape and load into the model.](images/clipboard-4080018206.png){#fig-working-iLand-splash width="514"}
...
The main side panel (typically on the left side of the window) contains two "windows" that are organized as tabs: The "legend" shows the legend for the current visualization and a ruler, while the "Javascript editor" tab let you access the JavaScript engine.

### Legend

![The legend shows either continuous or categorical data and a ruler to indicate the zoom level.](images/clipboard-1147054463.png){#fig-working-legend width="375"}
...
**(3)** The ruler shows the scale of the current visualization in meters.

### Javascript editor tab

![The Javascript editor tab allows working on JavaScript code that facilitates iLand objects and functions.](images/clipboard-3240416411.png){#fig-working-javascripteditor}
...
![The "visualization options" tab of iLand that control what and how information is displayed in the model.](images/clipboard-3404035556.png){#fig-working-visualization-options width="332"}

Section (1) are basic options, (2) show additional layers provided by sub models (e.g, disturbance modules), (3) allows to visualize user defined expressions, and (4) allows to focus on specific species.
...
::: callout-tip
Clicking on an element in the visualization (e.g., a tree, or a grid cell) shows details of the respective object in the "View Details" window (see (5) in @fig-working-ui-overview). The details show (in most cases) all the available variables, that can again used for expressions.
...
The following figure exemplifies ways on how iLand can be used to visualized different types of data:
...
iLand contains a little function plotter tool, that is based on the expression engine of iLand. You can plot arbitrary functions using the syntax and built-in functions of the [expression engine](https://iland-model.org/expression) (such as `polygon()`, or `if()`). The first unknown symbol encountered is treated as the predictor variable (you get a warning when variables further variables are in the expression). The function plotter can be accessed via the menu ( `Misc->Expression plotter`) or from the settings editor. The tool can be very helpful for tweaking allometric or response functions often used for species parameters.
