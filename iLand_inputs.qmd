...
-   contains the raster of the simulation landscape plus neighboring forests with cell values indicating the stand-ID, the stand-ID can be linked to information on vegetation
-   For more information see chapter on "Stand grid" (#sec-stand-grid)\
    node in project-file: *model.world.standGrid.fileName*


<span class=highlight-orange>**Optional**</span>
...
* iLand calculates maps with slope and aspect (more information [here](https://iland-model.org/simulation+extent?highlight=elevation#digital_elevation_model "iLand wiki: DEM")). When no DEM is given, a flat topography is assumed;\
...
* only needed if the [wind module](https://iland-model.org/wind+module) is used, resolution of 100x100 m, scales the global wind pattern to a local value, values per resource unit;\
...
**Climate databases**\
* .sqlite-files 
* contain tables with daily climate variables, table names can be linked to resource units via the environment file (for more information on the environment file see #sec-biophysical-landscape); one .sqlite-file per climate scenario;\
...
**Species databases**\
* .sqlite-file 
* contains the species parameter for the simulated tree species, region-specific \* more information [here](https://iland-model.org/species+parameter "iLand wiki: species parameter"));\
...
* contains for each resource unit the values for soil and climate conditions (for more information on the environment file see #sec-biophysical-landscape);\
...
**Initialization file**\
* CSV or other text format 
* contains information about the number of trees of a certain species that will be initialized on a specific stand-ID. \* see chapter #sec-initializing-with-forest-inventory-data;\
node in project file: *model.initialization.file*

**Snapshot**\
* .sqlite-file
* full representation of the internal vegetation and soil state of a landscape see chapter #sec-spinup;\
